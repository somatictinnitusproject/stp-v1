...
export default function StickyBar({ classification = "a" }) {
  const signupUrl = `https://somatictinnitusproject.com/signup?result=${classification}`;

  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-dark px-6 py-2.5 flex items-center justify-between gap-4 flex-wrap shadow-[0_-4px_24px_rgba(0,0,0,0.15)]">
      <div className="flex flex-col gap-0.5">
        <span className="text-[17px] font-bold text-white leading-none tracking-tight">
          You now know what's driving it.
        </span>
        <span className="text-[12px] text-white/70">Start the framework — free to join</span>
      </div>
